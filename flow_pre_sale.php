...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    {
    	//根据预售已改
        clear_cart(CART_PRE_SALE_GOODS);//增加购物车选择性结算--青蜂网络www.0769web.net
    }
...
elseif ($_REQUEST['step'] == 'checkout')
{
   ...
   // else if($_SESSION['extension_code'] == 'virtual_good')
    ...
    {
        /* 如果不完整则转向到收货人信息填写界面 */
		/* 代码修改_start  By  www.0769web.net  注释或删掉  */
        //ecs_header("Location: flow_pre_sale.php?step=consignee\n");
        //exit;
	...
	*/
        ...
        && trim($_CFG['invoice_content']) != '' && $flow_type != CART_EXCHANGE_GOODS)
    {
        ...
        foreach ($_CFG['invoice_type']['type'] as $key => $type)
        {
	    	...
	    	/*增值税发票_更改_END_www.0769web.net*/
        }
        $smarty->assign('inv_type_list', $inv_type_list);
		/*增值税发票_添加_START_www.0769web.net*/
		$smarty->assign('province_list', get_regions(1, $_CFG['shop_country']));
		/*增值税发票_添加_END_www.0769web.net*/
    }
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
	    ...
	    $order = array(
	        ...
	        /*增值税发票_删除_START_www.0769web.net*/
        	//'inv_type'        => $_POST['inv_type'],
        	//'inv_payee'       => trim($_POST['inv_payee']),
        	//'inv_content'     => $_POST['inv_content'],
			/*增值税发票_删除_END_www.0769web.net*/
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    /* 插入订单商品 下面这个SQL有修改 by www.0769web.net 注意末尾那个字段 */
	    /* 代码增加_start  By www.0769web.net */
	...
	//$GLOBALS['db'] -> query("unlock tables");
	    ...
	    if ($order['order_amount'] <= 0)
	    {
	        ...
	        foreach ($res AS $row)
	        {
                    /* 代码增加_start  By www.0769web.net _sunlizhi*/
...
//                    }
                    ...
                    /* 代码增加_end  By www.0769web.net _sunlizhi*/
...
elseif ($_REQUEST['step'] == 'add_package_to_cart')
{
    ...
    {
        /* 添加到购物车 修改 by www.0769web.net 增加两个参数 */
...
{
   /* 代码修改_start  By  www.0769web.net  将这块替换掉*/
