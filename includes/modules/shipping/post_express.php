...
class post_express
{
    ...
    function getorder($order){
		$kuaidi = sprintf($GLOBALS['_CFG']['kuaidi100'],'youzhengguonei',$order);
        $result = getcontent($kuaidi); 
        $getdata = json_decode($result,true);
        if($getdata['message']!='ok'){
            $data['data'][0] = array('time'=> local_date('Y-m-d H:m:s', gmtime()),'context'=>'查询失败,请联系管理员核查！');
        }else{
