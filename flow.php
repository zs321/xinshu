...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	if ( $row_xg['buymax'] >0 && $row_xg['sale_start_date'] < $time_xg_now  && $row_xg['sale_end_date'] > $time_xg_now  )
	{
		...
		{
			$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//增加购物车选择性结算--青蜂网络www.0769web.net
			$num_cart_old_1=$GLOBALS['db']->getOne("select sum(goods_number) from ". $GLOBALS['ecs']->table('cart') ." where $sql_where and goods_id= " . $goods->goods_id );//修改购物车选择性结算--青蜂网络www.0769web.net
...
elseif ($_REQUEST['step'] == 'del_in_cart_combo') //删除购物车项目 by mike
{
    ...
    if($goods->parent == 0){
        ...
        $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE $sql_where".
                " AND goods_id = '" . $goods->goods_id . "' AND group_id = '" . $goods->group . "'";//修改购物车选择性结算--青蜂网络www.0769web.net
        $GLOBALS['db']->query($sql);
        //更新临时购物车（删除配件）
        $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE $sql_where".
                " AND parent_id = '".$goods->goods_id."' AND group_id = '" . $goods->group . "'";//修改购物车选择性结算--青蜂网络www.0769web.net
        $GLOBALS['db']->query($sql);
    }else{
        //更新临时购物车（删除配件）
        $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE $sql_where".
...
elseif ($_REQUEST['step'] == 'add_to_cart_group') //套餐添加到购物车 by mike
{
    ...
    //批量加入购物车
    $sql = "SELECT rec_id FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE $sql_where" .//修改购物车选择性结算--青蜂网络www.0769web.net
   ...
   // var_dump($res);//exit;
    ...
    if($res){
        ...
        $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE ".
                " $sql_where AND group_id = '" . $group . "'";//修改购物车选择性结算--青蜂网络www.0769web.net
        $GLOBALS['db']->query($sql);
        //插入新的数据
...
elseif ($_REQUEST['step'] == 'login')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .
                " WHERE $sql_where AND is_gift > 0";//修改购物车选择性结算--青蜂网络www.0769web.net
        if ($db->getOne($sql) > 0)
        {
    ...
    else
    {
        ...
        if (!empty($_POST['act']) && $_POST['act'] == 'signin')
        {
            ...
            if ($user->login($_POST['username'], $_POST['password'],isset($_POST['remember'])))
            {
				...
				$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//增加购物车选择性结算--青蜂网络www.0769web.net
                $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') . " WHERE $sql_where ";//修改购物车选择性结算--青蜂网络www.0769web.net
                if ($db->getOne($sql) > 0)
                {
...
elseif ($_REQUEST['step'] == 'ajax_drop_goods')
{
    ...
    else
    {
        ...
        $result['total_number'] =$cart_goods['total']['total_number'];//修改青蜂网络购物车选择性结算
    }
...
elseif ($_REQUEST['step']== 'ajax_update_cart')
{
	...
	/* 秒杀代码增加_end  By www.0769web.net */
		...
		//查询：
		$sql = "SELECT `goods_id`, `goods_attr_id`,`product_id`, `extension_code` FROM" .$GLOBALS['ecs']->table('cart')." WHERE rec_id='$key' AND $sql_where";//修改购物车选择性结算--青蜂网络www.0769web.net
		...
		$sql = "SELECT b.goods_number,b.rec_id FROM ".$GLOBALS['ecs']->table('cart') . " a, ".$GLOBALS['ecs']->table('cart') . " b WHERE a.rec_id = '$key' AND $sql_wherea AND a.extension_code <>'package_buy' AND b.parent_id = a.goods_id AND $sql_whereb";//修改购物车选择性结算--青蜂网络www.0769web.net
		...
		if ($val > 0)
		{
			...
			while ($offers_accessories_row =$GLOBALS['db']->fetchRow($offers_accessories_res))
			{
				if ($row_num > $val)
				{
					...
					$GLOBALS['db']->query($sql);//修改购物车选择性结算--青蜂网络www.0769web.net
				}
				$row_num ++;
			}
			/*处理超值礼包*/
			...
			{
				//更新购物车中的商品数量
				$sql = "UPDATE ".$GLOBALS['ecs']->table('cart')." SET goods_number= '$val' WHERE rec_id='$key' AND $sql_where";//修改购物车选择性结算--青蜂网络www.0769web.net
			}
			/*处理普通商品或非优惠的配件*/
			else
			{
				...
				//更新购物车中的商品数量
				$sql = "UPDATE ".$GLOBALS['ecs']->table('cart')." SET goods_number= '$val', goods_price = '$goods_price' WHERE rec_id='$key' AND $sql_where";//修改购物车选择性结算--青蜂网络www.0769web.net
			}
		}
		//订货数量等于0
		else
		{
			...
			$sql = "DELETE FROM ".$GLOBALS['ecs']->table('cart')." WHERE rec_id='$key' AND $sql_where";//修改购物车选择性结算--青蜂网络www.0769web.net
		}
		$GLOBALS['db']->query($sql);
		/*删除所有赠品*/
...
elseif ($_REQUEST['step'] == 'update_cart')
{
	...
	if ( $row['buymax'] >0 && $row['sale_start_date'] < $time_xg_now  && $row['sale_end_date'] > $time_xg_now  )
	{
		if ( $val > intval($row['buymax']) )
		{
			show_message("此商品为秒杀限购商品，限购期间每人最多只能订购". $row['buymax'] . " 件！", $_LANG['back_to_cart'], 'flow.php');
			exit;
		}
		else
		{
			...
			{
				$num_else=intval($row['buymax'])-$num_cart_old;
				$message = "注意：<br> 商品 <font color=#330099>【".$row['goods_name']."】</font>  秒杀期间每人限购 <font color=#330099>". $row['buymax'] . "</font> 件<br>";
				if ($num_cart_old)
				{
...
/* 代码修改购物车选择性结算_end  By  www.0769web.net  */
        ...
        else
        {
            ...
            $sql = "DELETE FROM " .$GLOBALS['ecs']->table('cart').
                //" WHERE rec_id='$key' AND session_id='" .SESS_ID. "' AND group_id=''"; //青蜂网络购物车选择性结算注释
                " WHERE rec_id='$key' AND group_id='' AND $sql_where1"; //增加购物车选择性结算--青蜂网络www.0769web.net
        }
