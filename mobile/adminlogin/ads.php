...
/* 青蜂网络修改增加代码start */
	$filter['ad_i']    = empty($_REQUEST['ad_i']) ? 0 : trim($_REQUEST['ad_i']);

	$filter['ad_n']    = empty($_REQUEST['ad_n']) ? '' : trim($_REQUEST['ad_n']);
	$filter['ad_p']    = empty($_REQUEST['ad_p']) ? '' : trim($_REQUEST['ad_p']);
	...
	if( $filter['ad_i'] > 0)
	{
		$where .= " AND ad.ad_id = '".$filter['ad_i']."' ";
	}

	if(!empty($filter['ad_n']))
	{
		$where .= " AND ad.ad_name LIKE '%" . mysql_like_quote($filter['ad_n']) . "%' ";
	}

	if (!empty($filter['ad_p']))
	{
		$where .= " AND p.position_name LIKE '%" . mysql_like_quote($filter['ad_p']) . "%' ";
	}
/* 青蜂网络修改增加代码end */
...
/* 青蜂网络修改增加代码start */
    /* 获得总记录数据 */
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('touch_ad'). ' AS ad ' .
		   'LEFT JOIN ' . $GLOBALS['ecs']->table('touch_ad_position'). ' AS p ON p.position_id = ad.position_id '.$where;
/* 青蜂网络修改增加代码end */
