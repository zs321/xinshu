...
/* JS代码增加_start  By www.0769web.net */
...
function addToCart(goodsId, parentId, isnowbuy, extCode) {
	...
	if (isSelectAttr(spec_arr)) {
		...
		//Ajax.call('flow_pre_sale.php?step=add_to_cart', 'goods=' + $.toJSON(goods), addToCartResponse, 'POST', 'JSON');
        Ajax.call('flow_pre_sale.php?step=add_to_cart', 'goods=' + JSON.stringify(goods), addToCartResponse, 'POST', 'JSON');
	}

}

/* 组合购买__添加商品到购物车__Start By www.0769web.net */
function addToCartNums(goodsId, parentId) {
	...
	for (i = 0; i < goodsIds.length; i++) {
		...
		if (i == buynum) {
			...
			Ajax.call('flow_pre_sale.php?step=add_to_cart', 'goods=' + $.toJSON(goods), '', 'POST', 'JSON');
		}

	}
}
/* 组合购买__添加商品到购物车__End By www.0769web.net */
...
//			document.getElementById('tell_me_form').style.display = 'block';
		...
		var cart_url = 'flow_pre_sale.php?step=cart';
		//var cart_url = 'flow_pre_sale.php?step=cart';
		if (cartInfo) {
...
function MoveBox(gid) {
	...
	if (obj1.length > 0) {
		...
		// 购物车页面加入操作，刷新页面
		location.href = 'flow_pre_sale.php?step=cart';
	}
}
...
/* 代码修改_start 整个替换掉即可 By www.0769web.net */
/*******************************************************************************
 * 评论的翻页函数
 */
 ...
 * 添加礼包到购物车
 */
/* 代码修改_start By www.0769web.net 这里增加了一个变量 */
function addPackageToCart(packageId, indexId) {
	...
	/* 代码增加_end By www.0769web.net */
	Ajax.call('flow_pre_sale.php?step=add_package_to_cart', 'package_info=' + $.toJSON(package_info), addPackageToCartResponse, 'POST', 'JSON');
}
/* 代码修改_end By www.0769web.net */
/*******************************************************************************
 * 处理添加礼包到购物车的反馈信息
...
function opencartDiv(price, name, pic, goods_brief, goods_id, total, number) {
	...
	html += "<div class=cartbntfloat ><a href='flow_pre_sale.php'><img src='themes/68ecshopcom_360buy/images/jsico1.gif'></a><a href=\'javascript:cancel_div()\'><img src='themes/68ecshopcom_360buy/images/goon_ico1.gif'></a></div>";
	html += '</div></div>';
