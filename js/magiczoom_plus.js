...
function changeAtt(t,goods_id) {
	...
	changePrice();    // By www.0769web.net
...
function show_attr_status(theid, goods_id, attr_id_www_mb5_com_cn)
{
	...
	document.getElementById('spec_attr_type_'+www_mb5_com_cn).value=selected_first[0];

	//if(attr_selected()){
	//	return true;
	//}
	if(change_classname(theid,www_mb5_com_cn)){
		//var formBuy = document.forms['ECS_FORMBUY'];
		//spec_arr = getSelectedAttributes(formBuy);
		//Ajax.call('goods.php?act=get_products_info', 'id=' + spec_arr+ '&goods_id=' + goods_id, shows_number, 'GET', 'JSON');
		changePriceAll();
	...
	if (theid.parentNode.id.replace('catt_','')==attr_id_www_mb5_com_cn)
	{
