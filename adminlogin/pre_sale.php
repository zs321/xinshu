...
function action_edit ()
{
	...
	if($pre_sale_id <= 0)
	{
		...
		/* 当删除商品后再编辑预售报错解决方案--【模-板-之-家】修复 */
		$sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_activity') . " WHERE act_id='$pre_sale_id'";
		$goodsid = $GLOBALS['db']->getOne($sql);
		$sql = "SELECT goods_name FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id='$goodsid'";
		$editpre = $GLOBALS['db']->getOne($sql);
		if (empty($editpre))
		{
			sys_msg('当前预售中的商品不存在，请删除本预售信息后重新添加！', 1);
			exit;
		}
		...
		if($pre_sale['total_order'] > 0)
		{
			...
			while($row = $db->fetchRow($res))
			{
				...
				if($order['surplus'] + $order['money_paid'] >= $pre_sale['deposit'])
				{
                    ...
                    mb5_pay_log($order_id,$order['order_amount']);//处理预售支付尾款不能正常的问题
...
function action_remove ()
{
	...
	if(isset($_REQUEST['act_id']))
	{
		...
		/* 当删除商品后再删除预售报错解决方案--【模-板-之-家】修复 */
		$sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_activity') . " WHERE act_id='$id'";
		$goodsid = $GLOBALS['db']->getOne($sql);
		$sql = "SELECT goods_name FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id='$goodsid'";
		$delpre = $GLOBALS['db']->getOne($sql);
		if (empty($delpre))
		{
			/* 删除预售活动 */
			$sql = "DELETE FROM " . $GLOBALS['ecs']->table('goods_activity') . " WHERE act_id = '$id' LIMIT 1";
			...
			sys_msg(sprintf($_LANG['batch_drop_success'], 1), 0, $links);
			exit;
		}
