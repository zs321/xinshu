...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    else
    {
		...
		/* 当删除商品后再编辑团购报错解决方案--【模-板-之-家】修复 */
		$sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_activity') . " WHERE act_id='$group_buy_id'";
		...
		if (empty($editpre))
		{
			sys_msg('当前团购中的商品不存在，请删除本团购信息后重新添加！', 1);
			exit;
		}
        ...
        if ($group_buy['total_order'] > 0)
        {
            ...
            while ($row = $db->fetchRow($res))
            {
                ...
                if ($order['surplus'] + $order['money_paid'] >= $group_buy['deposit'])
                {
                    ...
                    mb5_pay_log($order_id,$order['order_amount']);//处理团购支付尾款不能正常的问题
